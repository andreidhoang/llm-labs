...
_GROUPED_MM_ALIGN = 8  # BF16 16-byte alignment requirement for torch._grouped_mm
...
def _run_experts_grouped_mm(w_up, w_down, x, num_tokens_per_expert):
    ...
    torch._grouped_mm requires the per-group token count to be a multiple
    of 8 (BF16 16-byte alignment). Random/synthetic routing rarely satisfies
    this, so we pad each expert's chunk to multiple of 8 with zero rows,
    call grouped_mm on the padded data, then slice off the padding rows.

    Padding overhead: at most 7 zero-rows per expert per call. With 8 experts,
    that's at most 56 extra rows out of typical 32K+ tokens — negligible.
    ...
    Returns:
        output: (total_tokens, dim) - same shape as input x
    """
    A = _GROUPED_MM_ALIGN
    T, D = x.shape
    # Per-expert padding to make each chunk a multiple of A
    pad_per_expert = (-num_tokens_per_expert) % A                  # (E,) values in [0, A)
    padded_n = num_tokens_per_expert + pad_per_expert              # (E,) all multiples of A
    total_pad = pad_per_expert.sum()                               # 0-D tensor
    T_padded = T + total_pad                                       # 0-D tensor (dynamic)

    # Build target index in padded layout: original token i in [0, T) maps to
    # position (i + cumulative_padding_inserted_BEFORE_its_expert) in the padded buffer.
    orig_cum = torch.cumsum(num_tokens_per_expert, dim=0)          # (E,)
    # cum_pad_before[e] = sum(pad_per_expert[0..e-1]); cum_pad_before[0] = 0
    cum_pad_before = torch.cumsum(pad_per_expert, dim=0) - pad_per_expert  # (E,)
    token_idx = torch.arange(T, device=x.device, dtype=orig_cum.dtype)
    # Each token's expert id via binary search on the cumulative original offsets.
    # right=True so token at position orig_cum[e]-1 maps to expert e (not e+1).
    expert_id = torch.searchsorted(orig_cum, token_idx, right=True)        # (T,)
    ...
    padded_offsets = torch.cumsum(padded_n, dim=0).to(torch.int32)

    # Cast weights once
    w_up_bf16 = w_up.bfloat16().transpose(-2, -1)
    w_down_bf16 = w_down.bfloat16().transpose(-2, -1)

    # Up-project: (T_padded, D) → (T_padded, expert_hidden_dim)
    h = torch._grouped_mm(x_padded, w_up_bf16, offs=padded_offsets)
    h = F.relu(h).square()
    # Down-project: (T_padded, expert_hidden_dim) → (T_padded, D)
    out_padded = torch._grouped_mm(h.bfloat16(), w_down_bf16, offs=padded_offsets)

    # Slice off padding: gather the original T positions back
    out = out_padded.index_select(0, target_idx)
    return out.type_as(x)


@torch.compiler.disable
def _run_experts_for_loop(w_up, w_down, x, num_tokens_per_expert):
    """Fallback for-loop implementation. Used on CPU/MPS, OR on CUDA when
    torch._grouped_mm is not available (e.g. torch < 2.7).

    Decorated with @torch.compiler.disable because .tolist() causes a device-host
    sync that torch.compile can't handle.

    Dtype handling: x can be BF16 (from cast embeddings), w_up/w_down are FP32
    (matrix params kept FP32 for Muon). Cast weights to x's dtype at the matmul
    boundary — same pattern as `_run_experts_grouped_mm` and `core._layers.Linear`.
    """
...
class ExpertGroup(nn.Module):
    ...
    def forward(self, x, num_tokens_per_expert):
        ...
        Returns:
            output:                 (T*K, dim)
        """
        # Use grouped_mm fast path on CUDA when the kernel is available
        # (torch._grouped_mm landed in torch 2.7+). The wrapper handles BF16
        # alignment by padding each expert's chunk to multiple of 8 — see
        # `_run_experts_grouped_mm` docstring. Set DISABLE_GROUPED_MM=1 to
        # force the for-loop fallback (debugging).
        import os
        if (x.is_cuda
                and hasattr(torch, "_grouped_mm")
                and os.environ.get("DISABLE_GROUPED_MM") != "1"):
