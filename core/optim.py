...
def adamw_step_fused(
    p: Tensor,              # (..., dim) - parameter tensor (any dtype)
    grad: Tensor,           # (..., dim) - gradient, same shape as p (any dtype)
    exp_avg: Tensor,        # (..., dim) - first moment, ALWAYS FP32
    exp_avg_sq: Tensor,     # (..., dim) - second moment, ALWAYS FP32
    ...
    """
    Fused AdamW step with FP32 master moments.

    Mixed-precision pattern: param p can be any dtype (FP32 for matrix params,
    BF16 for embeddings after the embedding cast), but exp_avg / exp_avg_sq
    are always FP32. Grad is upcast to FP32 inside this function so all the
    momentum / bias / divide math runs in FP32 regardless of param dtype.
    The final update is downcast back to p.dtype before in-place add.

    This avoids the BF16 lerp_(grad, scalar) dtype mismatch and gives the
    optimizer FP32 numerical headroom even on BF16 params.

    All in one compiled graph to eliminate Python overhead between ops.
    The 0-D CPU tensors avoid recompilation when hyperparameter values change.
    """
    # Promote grad to FP32 once at entry — all math below stays FP32.
    g = grad.float()
    # Weight decay (decoupled, applied before the update). Compute the scaling
    # factor in FP32 then cast to p's dtype for the in-place mul.
    p.mul_((1 - lr_t * wd_t).to(p.dtype))
    # Update running averages (lerp_ is cleaner and fuses well). All FP32.
    exp_avg.lerp_(g, 1 - beta1_t)
    exp_avg_sq.lerp_(g.square(), 1 - beta2_t)
    # Bias corrections (FP32 scalars).
    bias1 = 1 - beta1_t ** step_t
    bias2 = 1 - beta2_t ** step_t
    # Compute update in FP32, downcast to p.dtype only at the final in-place add.
    denom = (exp_avg_sq / bias2).sqrt() + eps_t
    step_size = lr_t / bias1
    update = (exp_avg / denom * (-step_size)).to(p.dtype)
    p.add_(update)
...
@torch.compile(dynamic=False, fullgraph=True)
def muon_step_fused(
    stacked_grads: Tensor,          # (K, ..., M, N) - stacked gradients (param dtype)
    stacked_params: Tensor,         # (K, ..., M, N) - stacked parameters (param dtype)
    momentum_buffer: Tensor,        # (K, ..., M, N) - first moment buffer, ALWAYS FP32
    second_momentum_buffer: Tensor, # (K, ..., M, 1) or (K, ..., 1, N) - second moment, ALWAYS FP32
    ...
    """
    Fused Muon step with FP32 master moments.

    momentum_buffer / second_momentum_buffer are ALWAYS FP32 regardless of
    stacked_params dtype. Grad is upcast to FP32 for momentum/variance math,
    Polar Express is intentionally run in BF16 for speed (the algorithm is
    stable at BF16 precision per the Muon papers), and the final update is
    written back to stacked_params at its native dtype.

    All in one compiled graph; 0-D CPU tensors avoid recompilation.
    """

    # Nesterov momentum in FP32 (matches FP32 momentum_buffer)
    g_fp32 = stacked_grads.float()
    momentum = momentum_t  # already FP32
    momentum_buffer.lerp_(g_fp32, 1 - momentum)
    g_fp32 = g_fp32.lerp_(momentum_buffer, momentum)

    # Polar express — intentionally BF16 for speed (algorithm stable at BF16)
    X = g_fp32.bfloat16() if COMPUTE_DTYPE == torch.bfloat16 else g_fp32
    ...
    if X.size(-2) > X.size(-1):  # Tall matrix
        ...
        for a, b, c in polar_express_coeffs[:ns_steps]:
            ...
            B = b * A + c * (A @ A)
            X = a * X + B @ X
    g = X.float()  # back to FP32 for variance reduction

    # Variance reduction (FP32 throughout — second_momentum_buffer is FP32)
    beta2 = beta2_t  # already FP32
    ...
    g = g * final_scale  # FP32 update

    # Cautious weight decay + parameter update — cast back to param dtype only here
    lr = lr_t
    wd = wd_t
    mask = (g * stacked_params.float()) >= 0
    update = (lr * g + lr * wd * stacked_params.float() * mask).to(stacked_params.dtype)
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def _step_adamw(self, group: dict) -> None:
        ...
        for p in group['params']:
            ...
            # State init — FP32 master moments regardless of param dtype
            if not state:
                state['step'] = 0
    ...
    def _step_muon(self, group: dict) -> None:
        ...
        # Momentum for every individual parameter — FP32 master buffer
        if "momentum_buffer" not in state:
            state["momentum_buffer"] = torch.zeros(num_params, *shape, dtype=torch.float32, device=device)
        momentum_buffer = state["momentum_buffer"]

        # Second momentum buffer is factored, either per-row or per-column.
        # For 3D weights (e.g. MoE w1: (num_experts, expert_dim, n_embd))
        # we must preserve all leading dims so the buffer broadcasts with
        # v_mean shape (num_params, *extra, ...). Always FP32.
        if "second_momentum_buffer" not in state:
            extra = shape[:-2]  # () for 2D, (num_experts,) for 3D MoE weights
            if shape[-2] >= shape[-1]:
                state_shape = (num_params, *extra, shape[-2], 1)
            else:
                state_shape = (num_params, *extra, 1, shape[-1])
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def _compute_muon(self, group: dict, info: dict, gather_list: list, rank: int) -> None:
        ...
        # Get or create group-level state — FP32 master buffers regardless of param dtype
        state = self.state[p]
        if "momentum_buffer" not in state:
            state["momentum_buffer"] = torch.zeros(chunk_size, *shape, dtype=torch.float32, device=device)
        if "second_momentum_buffer" not in state:
            # 3D MoE weights need all leading dims preserved (see _step_muon).
            extra = shape[:-2]
            if shape[-2] >= shape[-1]:
                state_shape = (chunk_size, *extra, shape[-2], 1)
            else:
                state_shape = (chunk_size, *extra, 1, shape[-1])
            state["second_momentum_buffer"] = torch.zeros(state_shape, dtype=torch.float32, device=device)
        red_dim = -1 if shape[-2] >= shape[-1] else -2
