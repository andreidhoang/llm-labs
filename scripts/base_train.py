...
from core.dataloader import tokenizing_distributed_data_loader_bos_bestfit, tokenizing_distributed_data_loader_with_state_bos_bestfit, synthetic_multimodal_loader
...
# Wallclock optimizations (port from nanochat v2 — see docs/wallclock_porting_vi.md)
parser.add_argument("--compile-fullgraph", action="store_true", help="torch.compile(fullgraph=True, mode='max-autotune'). Requires data-dependent branches eliminated (ve_zero placeholder enabled).")
...
parser.add_argument("--activation-ckpt", action="store_true", help="per-block activation checkpointing (saves ~17 GB HBM, costs ~33%% extra compute). Off by default — enable when memory-bound.")
# Model architecture
...
# MoE controls — defaults match GPTConfig (G≈2: 8 routed experts, top-2, 1 shared)
parser.add_argument("--num-experts", type=int, default=8, help="MoE: number of routed expert MLPs")
parser.add_argument("--top-k", type=int, default=2, help="MoE: number of routed experts active per token")
parser.add_argument("--num-shared-experts", type=int, default=1, help="MoE: number of always-active shared experts")
# Multimodal extension (default off → all existing text-only paths unchanged)
parser.add_argument("--multimodal", action="store_true", help="enable multimodal training (frozen SigLIP2 + scatter into LLM trunk)")
parser.add_argument("--mix-ratio", type=float, default=0.3, help="target token-level vision/text mix ratio (default 0.3)")
...
# Wallclock optimization toggles (see docs/wallclock_porting_vi.md)
if args.compile_fullgraph:
    model._use_compile_fullgraph_friendly = True
    print0(f"✓ Compile-fullgraph friendly mode enabled (zero-tensor ve placeholder)")
if args.activation_ckpt:
    model._use_activation_checkpointing = True
    print0(f"✓ Activation checkpointing enabled (per-block, use_reentrant=False)")
...
def disable_fp8(model):
    ...
    # Swap Float8Linear -> nn.Linear (the moe-branch nanochat removed the custom Linear
    # class; nn.Linear works for the FP8 swap path since we just need a standard module
    # to share weight tensors with).
...
_compile_kwargs = dict(dynamic=False, mode=args.compile_mode)
if args.compile_fullgraph:
    _compile_kwargs["fullgraph"] = True
print0(f"torch.compile(... {_compile_kwargs})")
...
# Multimodal: wrap text loader with synthetic image injection at mix_ratio.
# image_pad_token_id is set to a vocab id reserved for vision placeholders.
...
# (x, y, batch_extras, state) for multimodal where batch_extras has
# pixel_values/grid_thw/image_pad_mask/modality_mask. Fall back to {} for text.
if len(_first) == 4:
    x, y, batch_extras, dataloader_state_dict = _first
else:
    x, y, dataloader_state_dict = _first
    batch_extras = {}
...
while True:
    ...
    synchronize()
    t0 = time.time()
    # Per-modality loss accumulators (multimodal only; values stay 0 in text-only path)
    mm_loss_text_sum = torch.zeros((), device=device)
    mm_loss_vision_sum = torch.zeros((), device=device)
    mm_n_text = torch.zeros((), device=device, dtype=torch.long)
    mm_n_vision = torch.zeros((), device=device, dtype=torch.long)
    for micro_step in range(grad_accum_steps):
        # Multimodal: when dataloader returns extras (pixel_values, grid_thw, image_pad_mask,
        # modality_mask), thread them through. text-only batches have batch_extras={}.
        out = model(x, y, **batch_extras)
        if isinstance(out, dict):
            loss = out["loss"]
            # Track per-modality losses for logging (they accumulate across micro-steps)
            mm_loss_text_sum = mm_loss_text_sum + out["loss_text"].detach() * out["n_text"]
            mm_loss_vision_sum = mm_loss_vision_sum + out["loss_vision"].detach() * out["n_vision"]
            mm_n_text = mm_n_text + out["n_text"]
            mm_n_vision = mm_n_vision + out["n_vision"]
        else:
            loss = out
        ...
        if scaler is not None:
            ...
            loss.backward()
        _nxt = next(train_loader) # prefetch the next batch while the GPU is busy with forward/backward
        if len(_nxt) == 4:
            x, y, batch_extras, dataloader_state_dict = _nxt
        else:
            x, y, dataloader_state_dict = _nxt
            batch_extras = {}
    # step the optimizer
    ...
    if step % 100 == 0:
        ...
        if args.multimodal:
            ...
            n_vision_total = mm_n_vision.item()
            log_data["train/loss_text"] = (mm_loss_text_sum.item() / n_text_total) if n_text_total > 0 else 0.0
            log_data["train/loss_vision"] = (mm_loss_vision_sum.item() / n_vision_total) if n_vision_total > 0 else 0.0
            log_data["train/n_text"] = n_text_total
            log_data["train/n_vision"] = n_vision_total
            log_data["train/r_actual"] = n_vision_total / max(1, n_text_total + n_vision_total)
